...
from .atom_data import protein_res_dict
...
class GridDataset(BaseWrapperDataset):
    def __init__(
        ...
        assert self.args.data_type in [
            "molecule"
        ], f"{self.args.data_type} is not supported for now"

        self.init_process_func = {
            "molecule": self.get_molecule_basic_feat,
        }
    ...
    def get_grid_feat(self, atom_feat):
    ...
    def __cached_item__(self, index: int, epoch: int):
        with data_utils.numpy_seed(self.seed, epoch, index):
            ...
            atom_feat = self.init_process_func[data_type](data)
            ...
            feat = self.get_grid_feat(atom_feat)
    ...
    def get_molecule_basic_feat(self, data):
