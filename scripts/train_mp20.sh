...
torchrun --nproc_per_node=$n_gpu --nnodes=$OMPI_COMM_WORLD_SIZE  --node_rank=$OMPI_COMM_WORLD_RANK  --master_addr=$MASTER_IP --master_port=$MASTER_PORT \
      ...
      --grid-len 0.24  --gzip --H-prob 1.0 --xyz-resolution 0.01 --recycle 2 \
      --loss-ratio-tree 1.0 --loss-ratio-atom 0.1 --loss-ratio-xyz 0.1 --loss-ratio-count 1.0 \
      --atom-type-key atom_type --atom-pos-key atom_pos --lattice-matrix-key lattice_matrix --allow-atoms all  --head-dropout 0.1 \
      --tree-delete-start-layer 1 --tree-delete-ratio 0.1  --max-num-atom 128 --random-rotation-prob 0.5 --crystal-random-shift-prob 0.5 \
      $more_args \
